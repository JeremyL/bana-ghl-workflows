...
*Last edited: 2026-04-02 · Last reviewed: 2026-04-01*
...
| Cold Drip double-trigger on Lost              | New Leads     | WF-Cold-Drip-Monthly / WF-Dispo-Re-Engage    | WF-Cold-Drip-Monthly triggers on LT FU: Lost stage entry, but WF-Dispo-Re-Engage also explicitly enrolls into WF-Cold-Drip-Monthly after moving to that stage (Steps 2 + 3). Verify GHL deduplicates and the contact is not enrolled twice. If not, remove explicit enrollment from WF-Dispo-Re-Engage or remove the Lost stage trigger from WF-Cold-Drip-Monthly.                                                                                                |
| ~~Email casing — normalize to lowercase~~     | ~~Both~~      | ~~Import / n8n intake~~                      | **RESOLVED 2026-04-02.** Email normalization added to n8n intake workflow. Lowercase applied at Step 2C (search term), Step 4A (confirmed + unconfirmed emails from Prospect Data), and Step 4B (confirmed email from webhook). All email data entering New Leads is lowercased before GHL API calls. Prospect Data stays in original case — no normalization needed there. See Resolved Notes below.                                                              |
...
### Resolved Notes (2026-04-02) — Email Case Normalization

- **E1 → Resolved.** Email normalization added to n8n intake workflow (intake-workflow.md). Three touchpoints: Step 2C lowercases webhook email before searching Prospect Data's `All Emails` field (case-insensitive matching without changing stored data). Step 4A lowercases both the confirmed webhook email and all unconfirmed Owner N Email 1–4 values from the Property record before mapping to Contact fields. Step 4B lowercases the webhook email for no-match leads. Prospect Data retains original case — normalization happens only at the boundary when data enters New Leads.

---

### Resolved Notes (2026-04-01) — Owner 2/3 Fields & Template Count

- **O5 → Resolved.** Verified 10 NL-SMS templates (NL-SMS-01 through NL-SMS-10) in messaging.md master table. Fixed quick-reference.md: NL-SMS (9) → (10), SMS subtotal 27 → 28, grand total 50 → 51.
- **O6 → Resolved.** pipeline.md line 402: "ACQUISITIONS" → "QUALIFIED".
- **O7 → Resolved.** workflows.md line 252: removed "LT FU: Nurture leads (stalled deals)" from Applies to. Nurture leads use WF-Nurture-Monthly, not WF-Cold-Drip-Monthly.
- **O8 → Resolved.** messaging.md lines 311-312: removed Nurture from Stage and Applies to in Cold Monthly header.
- **O9 → Resolved.** for-review.md Verified section: "all 50 template IDs" → "all 51 template IDs".
- **O10 → Resolved.** ROLE.md Last edited: 2026-03-30 → 2026-04-02.
- **O11 → Resolved.** Improvement #21: "5-min target" → "10-min target".
- **O12 → Resolved.** Updated stale dispo references in improvements #9, #10, #25 to use Lost/Abandoned status terminology.
- **O13 → Resolved.** Marked "Concatenated search field" as completed in n8n/intake-workflow.md Future Enhancements.
- **O15 → Resolved.** Verified via API: Offer Price % is TEXT in both GHL accounts. Fixed prospect-data/data-model.md: Number → Text.
- **O2 → Resolved.** FUB "Listing Price" = accidental data in FUB native MLS field. Do not import. Full FUB → GHL mapping created: prospect-data/fub-import-mapping.md.
- **O3 → Resolved.** ~~workflows.md Step 4: "Wait: 3 days" → "Wait: 7 days" between WR-EMAIL-04 and WR-EMAIL-05, matching sequences.md (Day 14 → Day 21).~~ **Superseded 2026-04-02:** Cold Email removed — WR-EMAIL templates deleted.
- **O14 → Resolved.** n8n/intake-workflow.md Rate Limits: updated "18 sequential queries" → "3 queries max (ref → phone → email)" using concatenated fields.
- **O16 → Resolved.** Added Cold Drip double-trigger verification to Pre-Launch Verifications table.
- **O17 → Resolved.** ROLE.md banned phrases now includes slash variants matching messaging.md: "following up," "check-in."
- **O1 → Resolved.** Owner 2 and Owner 3 custom fields (24 each, 48 total) created in Prospect Data via GHL API. All names, types, and fieldKeys match Owner 1.
- **O4 → Resolved.** prospect-data/data-model.md updated: "Same 20 fields" → "Same 24 fields" for both Owner 2 and Owner 3.
...
### Resolved Notes (2026-04-07) — Pipeline Merge & Renumber

- **P7:** Leads + Qualified merged into single "01 : Acquisition" pipeline (10 stages: New Leads, Day 1-10, Day 11-30, Comp, Make Offer, Negotiations, Additional Info Needed, Contract Sent, Contract Signed, Nurture). "Comps/Pricing" renamed to "Comp". Nurture trigger stage preserved in Acquisition (still auto-moves to LT FU: Nurture). Pipelines renumbered: DD 03→02, VA 04→03, LT FU 05→04. New pipeline added: 05 : Disposition (post-acquisition sales — selling properties Bana Land has already purchased, TBD stages). Qualification is now intra-pipeline (Day 11-30 → Comp within Acquisition, no longer cross-pipeline). All files updated: pipeline.md, data-model.md, workflows.md, rules.md, sequences.md, quick-reference.md, ROLE.md, README.md, validate-new-leads.sh, n8n/intake-workflow.md, memory files.
...
- **C1:** ~~WF-Cold-Email-Subflow split into P1/P2.~~ **Superseded 2026-04-02:** Cold Email dropped as a lead source. Both WF-Cold-Email-Subflow-P1 and P2 deleted. All Cold Email workflows, templates (WR-*), and `cold: email only` tag removed. 12 → 10 workflows, 51 → 45 templates, 7 → 6 sources.
...
- **H6:** ~~Added `cold: email only` tag removal to re-submission cleanup.~~ **Superseded 2026-04-02:** Cold Email removed — tag and all references deleted.
...
- **M3:** ~~Template count corrected: 51.~~ **Superseded 2026-04-02:** Now 45 templates (6 WR-* templates removed with Cold Email).
...
- **M7:** ~~Added 1-hour move guidance for Cold Email no-phone leads in WF-New-Lead-Entry notification.~~ **Superseded 2026-04-02:** Cold Email removed — no-phone branch deleted from WF-New-Lead-Entry.
...
- **N-29:** Changed messaging.md Nurture Monthly header from "first touch immediate on entry" → "30-day wait before first touch." Matches sequences.md and workflows.md WF-Nurture-Monthly (Wait: 30 days).
- **N-30:** ~~Rewrote NURQ-EMAIL-03 in messaging.md to replace banned phrase.~~ (NURQ templates removed — merged into LTQ. See Decision Log #34.)
...
- **N-34:** ~~Replaced garbled "through" workflow groupings in workflows.md go-live checklist and README.md with explicit comma-separated list of all 12 workflows matching Step 6 index order.~~ **Superseded 2026-04-02:** Workflow list updated to 10 (Cold Email P1/P2 removed).
...
- **N-38:** ~~Changed pipeline.md Cold Email sub-flow from "runs concurrently with normal stage progression" → "runs concurrently with the Day 1–30 sequence (triggers on Day 1-10 entry)."~~ **Superseded 2026-04-02:** Cold Email removed — sub-flow section deleted from pipeline.md.
...
- **N-14:** Split WF-Cold-Drip-Monthly/WF-Nurture-Monthly into monthly + quarterly workflows. Quarterly later merged into single WF-Long-Term-Quarterly. See Decision Log #13, #34.
- **N-15:** Removed Improvement #2 (Multi-Property Cadence Overlap) entirely. See Decision Log #2.
- **N-16:** Renamed COLD-SMS templates to sequential order (01/02/03) across messaging.md, sequences.md, workflows.md.
...
- **N-20:** Changed WF-Cold-Drip-Monthly initial wait from 14d → 30d, added 30d initial wait to WF-Nurture-Monthly. Updated timing labels across sequences.md, messaging.md, and workflows.md. Both Cold and Nurture now have a 30-day breather before first touch.
...
- **N-26:** ~~Clarified pipeline.md Cold Email sub-flow — standard steps suppressed while WF-Cold-Email-Subflow is active.~~ **Superseded 2026-04-02:** Cold Email removed — sub-flow deleted.
- **N-27:** Aligned Nurture quarterly transition to match Cold — removed trailing 30-day wait from WF-Nurture-Monthly, added leading 90-day wait to quarterly. Both now: last monthly touch → enroll quarterly → 90-day wait → first quarterly touch. (Quarterly later merged into single WF-Long-Term-Quarterly. See Decision Log #34.)
...
- **Cold drip sequence (WF-Cold-Drip-Monthly monthly + WF-Long-Term-Quarterly)** — template references, timing (30-day initial wait + 14-day spacing) consistent across sequences.md, messaging.md, and workflows.md
...
- **Source tracking** — native Opportunity Source (immutable, set once — first-touch attribution) + Latest Source custom text field (updated on re-submission) + Latest Source Date. 6 source values: Cold Call, Cold SMS, Direct Mail, VAPI, Referral, Website. Source tags match these values. All 6 sources routed in WF-New-Lead-Entry/WF-Response-Handler. *(updated 2026-04-02: 7→6, Cold Email removed)*
- **Source → owner routing** — LM owns Cold SMS/Call; AM owns Direct Mail/VAPI/Referral/Website. Consistent across pipeline.md, sequences.md, workflows.md (WF-New-Lead-Entry, WF-Response-Handler), and ROLE.md. *(updated 2026-04-02: Cold Email removed from LM scope)*
...
- **Contact hours** — "9 AM – 7 PM local time" consistent across ROLE.md, rules.md §1, and workflows.md compliance checklist
...
- **Quick reference template-to-workflow mapping** — all 45 template IDs in messaging.md quick reference match their workflow assignments in workflows.md. *(updated 2026-04-02: 51→45, 6 WR templates removed with Cold Email)*
...
- **Workflow count** — 10 workflows confirmed across workflows.md, MEMORY.md, and README.md. *(updated 2026-04-02: 12→10, Cold Email P1/P2 deleted)*
- **Template-to-workflow mapping post-consolidation** — all 45 template IDs in messaging.md quick reference re-verified against workflow assignments in workflows.md. All match. *(updated 2026-04-02: 51→45)*
...
- Monthly report: count of Lost/Abandoned status changes — helps identify patterns
...
**Gap:** Abandoned + `abandoned: no longer own` has zero future outreach. These sellers sold the specific property we targeted, but may own other parcels.
...
| 36  | Data Model & Workflow Audit   | Pre-implementation audit found 17 issues (2 critical, 6 high, 8 medium, 1 deferred). All resolved. See audit resolution notes below. Trigger conventions standardized. Template count: 45. *(updated 2026-04-02: Cold Email P1/P2 removed)*                                                                                                                                                    | 2026-03-22 |
...
| 38  | Pipeline Merge — Acquisition  | Leads + Qualified merged into single "01 : Acquisition" pipeline (10 stages). Comps/Pricing → Comp. Nurture trigger stage preserved. Pipelines renumbered: DD 03→02, VA 04→03, LT FU 05→04. New pipeline 05 : Disposition (post-acquisition sales). Qualification is now intra-pipeline. All files updated (see P7 resolved note). | 2026-04-07 |
